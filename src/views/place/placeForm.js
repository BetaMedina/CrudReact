...
class UserForm extends AMFormComponent {
    ...
    // Render methods:
    render () {
        let title = (this.props.match && this.props.match.params.id !== 'new') ? 'Editar lugares' : ('Incluir Lugares');
        return (
            <Fragment>
                        ...
                        <Row>
                            <Col style={{ marginBottom: 20 }} md={4} xs={12}>
                                <TextField
                                    ...
                                    helperText={this.getErrorByField('address')}
...
const mapStateToProps = ({ placeReducer }) => {
    const { items, input, loading, error } = placeReducer;
    return { items, input, loading, error: error };
};

export default connect(mapStateToProps,
    {
        newObject: AppActions.place.newObject,
        getObject: AppActions.place.getObject,
        createObject: AppActions.place.createObject,
        saveObject: AppActions.place.saveObject,
        updateObject: AppActions.place.updateObject,
        inputChanged: AppActions.place.inputChanged,
        validateObject: AppActions.place.validateObject,
        prepareForm: AppActions.place.prepareForm,
        openDialog,
        openSnack,
        push
    }
