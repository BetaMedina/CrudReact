...
class UserList extends Component {
        ...
        let tableRows = items.map((item, key) => {
            return (
                <TableRow key={key}>
                    ...
                    <TableCell>{this.translateRole(item.role)}</TableCell>
                    <TableCell>{item.active ? <IconCheck /> : <div />}</TableCell>
    ...
    // Action methods:
    doInsert () {
        this.props.push('/usuarios/new/');
    }

    doEdit (e, item) {
        // e.stopPropagation();]
        this.props.history.push(`/usuarios/${item.id}`);
    }

    doDelete (e, item) {
        e.stopPropagation();
        // Dialog
        this.props.openDialog(`Deseja remover ${item.name} (${item.email}) ?`, '3', () => this.props.deleteObject(item.id));
    }
...
const mapStateToProps = ({ screenReducer, userReducer }) => {
    const { screenSize } = screenReducer;
    const { items, loading } = userReducer;
    // const {user} = auth;
...
export default connect(mapStateToProps,
    {
        getObjects: AppActions.user.getObjects,
        deleteObject: AppActions.user.deleteObject,
        openDialog,
        openSnack,
        push
    })(UserList);
